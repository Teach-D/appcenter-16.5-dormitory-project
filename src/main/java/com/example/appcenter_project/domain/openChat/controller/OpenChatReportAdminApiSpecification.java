...
public interface OpenChatReportAdminApiSpecification {
    ...
    @Operation(summary = "신고 목록 조회", description = "상태별 신고 목록을 최신순으로 페이징 조회. ROLE_ADMIN 전용.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "403", description = "관리자 권한 없음", content = @Content)
    })
    ResponseEntity<Page<ResponseOpenChatReportDto>> findReports(
            @Parameter(description = "신고 상태 (PENDING/APPROVED/CANCELLED)", example = "PENDING")
    ...
    @Operation(summary = "학번 기준 누적 신고 수", description = "특정 학번이 대상인 신고를 상태별로 집계. 재가입 고려하여 학번으로 집계함.")
    @ApiResponses({
    ...
    ResponseEntity<Long> countReports(
            ...
            @Parameter(description = "집계 상태", example = "APPROVED") @RequestParam ReportStatus status);
}